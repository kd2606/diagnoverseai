...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (data.session) {
      // If login originated from doctor flow, strictly push to doctor dashboard
      // The middleware will handle any hard fallback if needed, but we trust the flow here
  ...
  const handleGoogleLogin = async () => {
    ...
    await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: { redirectTo: `${window.location.origin}/dashboard/doctor` }
    });
  };
  ...
  const handleDemoLogin = async () => {
    ...
    setError(null);
    const demoEmail = process.env.NEXT_PUBLIC_CLINICIAN_DEMO_EMAIL;
    const demoPassword = process.env.NEXT_PUBLIC_CLINICIAN_DEMO_PASSWORD;
      ...
      >
        <div className="relative z-10">
          <div className="text-center mb-10">
            <h1 className="text-3xl font-semibold tracking-tight text-white mb-2">Welcome Back, Clinician</h1>
            <p className="text-white/40 text-[15px] leading-relaxed tracking-tight">Sign in to Clinician Console</p>
          </div>
          ...
          <p className="mt-8 text-center text-white/40 text-[13px]">
            ...
            <Link href="/auth/doctor/register" className="text-indigo-400 hover:text-indigo-300 font-medium transition-colors">
              Create one
            </Link>
          </p>
        </div>
