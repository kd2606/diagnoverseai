...
async function runScan(mode: ModeId, _image: string): Promise<Result> {
  ...
  const byMode: Record<ModeId, Result> = {
    face: {
      headline: "No acute facial asymmetry detected",
      severity: "clear",
      findings: [
        { label: "Symmetry index", confidence: 0.94, note: "Within normal range" },
        { label: "Pallor markers", confidence: 0.71, note: "Mild — recheck in daylight" },
        { label: "Periorbital swelling", confidence: 0.12, note: "Not detected" },
      ],
    },
    eye: {
      headline: "Mild conjunctival redness",
      severity: "watch",
      findings: [
        { label: "Conjunctival redness", confidence: 0.68, note: "Monitor for 48 h" },
        { label: "Scleral yellowing", confidence: 0.09, note: "Not detected" },
        { label: "Pupil response", confidence: 0.88, note: "Appears normal" },
      ],
    },
    skin: {
      headline: "Lesion warrants clinician review",
      severity: "review",
      findings: [
        { label: "Border irregularity", confidence: 0.74, note: "Asymmetric edges" },
        { label: "Pigment variation", confidence: 0.62, note: "Two-tone pattern" },
        { label: "Surface texture", confidence: 0.41, note: "Slightly raised" },
      ],
    },
  };
  return byMode[mode];
            ...
            {result ? (
              ...
              >
                <SpotlightCard accent={SEVERITY[result.severity].accent} className="p-6">
                  ...
                  <div className="mt-7 flex items-center gap-2 rounded-2xl border border-white/[0.05] bg-white/[0.02] p-3.5">
                    <CheckCircle2
