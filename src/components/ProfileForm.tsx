...
import { User, Activity, Mail, Phone, MapPin, Save, ShieldCheck, Lock } from "lucide-react";
...
export function ProfileForm({ role, initialData }: { role: "patient" | "doctor", initialData: any }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Email is intentionally excluded — it's managed by Supabase Auth
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <input
                ...
                disabled
                className="w-full bg-white/[0.01] border border-white/5 rounded-xl py-2.5 pl-10 pr-10 text-sm text-white/40 cursor-not-allowed"
              />
              <Lock className="absolute right-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-white/20" />
            </div>
            <p className="text-[11px] text-white/30">Managed by your login provider</p>
          </div>
